...
namespace src.model_store.rslt_objects
{
    ...
    public class reactionforce_list_store
    {
        ...
        public void update_buffer_data()
        {
            ...
            {
                load_max = (float)Math.Max(load_max, Math.Abs(reactionForce_data.reaction_x));
                load_max = (float)Math.Max(load_max, Math.Abs(reactionForce_data.reaction_y));
            }
            ...
            foreach (reactionforce_store reactionForce_data in reactionForceMap.Values)
            {
                ...
                int load_sign = reactionForce_data.reaction_x > 0 ? 1 : -1;
                float ld_visualization_factor = load_sign * load_size;
                float ld_scale = (float)(reactionForce_data.reaction_x / load_max) * load_size;
                ...
                // Add labels
                string label_string1 = FormatReactionForceValue((float)reactionForce_data.reaction_x);
            ...
            foreach (reactionforce_store reactionForce_data in reactionForceMap.Values)
            {
                ...
                int load_sign = reactionForce_data.reaction_y > 0 ? 1 : -1;
                float ld_visualization_factor = load_sign * load_size;
                float ld_scale = (float)(reactionForce_data.reaction_y / load_max) * load_size;
                ...
                // Create the reaction force label
                // Add labels
                string label_string1 = FormatReactionForceValue((float)reactionForce_data.reaction_y);
            ...
            reaction_force_label.update_buffer(gvariables_static.geom_size);
